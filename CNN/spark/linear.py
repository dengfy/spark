...
class LinearClassifier(Classifier):
  ...
  def backward(self, data, count):
    ...
    """
    L = softmax.map(lambda (x, (L, df)) : L).reduce(lambda a, b : a + b) # replace it with your code
    ...
    """
    dLdl1 = softmax.map(lambda (x, (L, df)) : linear_backward(df, x, self.A))
    """
    ...
    """
    dLdA = dLdl1.map(lambda (dx, dA, db) : dA).reduce(lambda a, b : a + b) # replace it with your code
    dLdb = dLdl1.map(lambda (dx, dA, db) : db).reduce(lambda a, b : a + b)# replace it with your code
